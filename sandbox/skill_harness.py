...
def validate_output(out) -> str | None:
    """
    Returns an error string if `out` doesn't satisfy the decide() output
    contract, else None. Shared by main()'s test-harness loop below AND by
    orchestrator/skill_runtime.py's re-validation of every REAL runtime
    call (not just promotion-time ones) - one copy of the rules, so they
    can't quietly drift apart between "what gets approved" and "what's
    trusted to actually run".
    """
    if not isinstance(out, dict):
        return f"decide() must return a dict, got {type(out).__name__}"
    action = out.get("action")
    if action not in ALLOWED_ACTIONS:
        return f"action {action!r} not in allowed set"
    if action == "say":
        text = out.get("text", "")
        if not isinstance(text, str) or not (0 < len(text) <= MAX_SAY_LEN):
            return "say text missing/too long"
    if action == "give_item":
        if not isinstance(out.get("item_id"), str) or \
           not isinstance(out.get("count"), int) or \
           not (1 <= out["count"] <= 8):
            return "give_item malformed or count out of bounds"
    try:
        json.dumps(out)
    except (TypeError, ValueError):
        return "action not JSON-serializable"
    return None
